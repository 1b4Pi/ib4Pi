...
export default function EffectMaterial({ swiper, on, extendParams }) {
  ...
  const setTranslate = () => {
    ...
    for (let i = 0; i < slides.length; i += 1) {
      ...
      if (isFloatSPV) progress = Math.floor(progress * 100) / 100;
      ...
      const scaleDiffExtra = isFloatSPV ? scaleDiff / 2 : 0;
        ...
        if (centeredSlides && slidesPerView > 1) {
          ...
          if (
            ...
            translate =
              swiperTranslate +
              currentSlideSize *
                (smallSlideRatioNormalize + scaleDiffExtra + scaleDiff) *
                (1 - currentProgress);
          }
      ...
      if (slidesPerView === 1) {
        ...
        if (isLargeToCurrent) {
          ...
          translate =
            swiperTranslate -
            (isFloatSPV ? (currentSlideSize * scaleDiff) / 2 : 0);
